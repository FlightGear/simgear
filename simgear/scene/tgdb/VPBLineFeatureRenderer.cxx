...
void VPBLineFeatureRenderer::applyLineFeatures(BufferData& buffer, osg::ref_ptr<SGReaderWriterOptions> options, osg::ref_ptr<SGMaterialCache> matcache)
{
    ...
    if (! matcache) {
        SG_LOG(SG_TERRAIN, SG_DEV_ALERT, "Unable to get materials library to generate roads");
        return;
    }    
    ...
    for (auto roads = _lineFeatureLists.begin(); roads != _lineFeatureLists.end(); ++roads) {
        ...
        for (LineFeatureBinList::iterator rb = roadBins.begin(); rb != roadBins.end(); ++rb)
        {
            ...
            if (!mat) {
                SG_LOG(SG_TERRAIN, SG_DEV_ALERT, "Unable to find material " << (*rb)->getMaterial() << " at " << loc << " " << bucket);
                continue;
            }    
            ...
            if (_useDecals) {
                for (auto r = lineFeatures.begin(); r != lineFeatures.end(); ++r) {
                    ...
                    if (r->_width > minWidth) generateLineFeatureTessellated(buffer, *r, buffer._transform->getMatrix(), v, t, n, rlocal, lights, x0, x1, ysize, light_edge_spacing, light_edge_height, light_edge_offset, elevation_offset_m);
                    if (rlocal->empty()) {
                        rlocal->unref();
                    } else {
                ...
                for (auto r = lineFeatures.begin(); r != lineFeatures.end(); ++r) {
                    if (r->_width > minWidth) generateLineFeatureTriangles(buffer, *r, buffer._transform->getMatrix(), v, t, n, lights, x0, x1, ysize, light_edge_spacing, light_edge_height, light_edge_offset, elevation_offset_m);
                }
            }
...
void VPBLineFeatureRenderer::generateLineFeatureTriangles(BufferData& buffer, LineFeatureBin::LineFeature road, osg::Matrix localToWorldMatrix, osg::Vec3Array* v, osg::Vec2Array* t, osg::Vec3Array* n, osg::Vec3Array* lights, double x0, double x1, unsigned int ysize, double light_edge_spacing, double light_edge_height, bool light_edge_offset, double elevation_offset_m)
{
    ...
    osg::Vec3d ma, mb;
    std::vector<osg::Vec3d> roadPoints;
...
void VPBLineFeatureRenderer::generateLineFeatureTessellated(BufferData& buffer, LineFeatureBin::LineFeature road, osg::Matrix localToWorldMatrix, osg::Vec3Array* v, osg::Vec2Array* t, osg::Vec3Array* n, osg::Vec2Array* roads, osg::Vec3Array* lights, double x0, double x1, unsigned int ysize, double light_edge_spacing, double light_edge_height, bool light_edge_offset, double elevation_offset_m)
{
    ...
    const int cols = elevationLayer->getNumColumns();
    const int rows = elevationLayer->getNumRows();
    float minElevation = 9000.0;
    float maxElevation = -9000.0;

    const int yMin = std::floor(bb.yMin()) - 1;
    const int yMax = std::ceil(bb.yMax()) + 1;
    const int xMin = std::floor(bb.xMin()) - 1;
    const int xMax = std::ceil(bb.xMax()) + 1;
